...
<script type="text/javascript">
	...
	$(function(){
		...
		$("[name=username]").change(function(){
			//准备请求信息
			var url = "Client/UserServlet?method=checkUsername";
			var params = {username:this.value};
			//发送ajax请求
			$.post(url,params,function(data){
				if(data==1){
					$("[name=username]").val("");
					$("[name=username]").attr("placeholder","此用户名已经被占用");
				}
			},"json");
		})
		...
		$("#code_img").click(function(){
			//刷新图片，实际上就是更改img标签的src属性值
			//base标签对js代码不起作用
			$(this).attr("src","${pageContext.request.contextPath}/code.jpg?t="+Math.random());
		})
...
<%-- 								<span class="errorMsg"><%=request.getAttribute("msg")==null?"":request.getAttribute("msg") %></span> --%>
							...
							<div class="form">
								...
								<form action="Client/UserServlet?method=regist" method="post">
									<label>用户名称：</label>
									<input class="itxt" value="${empty param.username?'':param.username}"  type="text" placeholder="请输入用户名" autocomplete="off" tabindex="1" name="username" />
									<br />
									<br />
									<label>用户密码：</label>
									...
									<br />
									<label>电子邮件：</label>
									<input class="itxt" value="${empty param.email?'':param.email}" type="text" placeholder="请输入邮箱地址" autocomplete="off" tabindex="1" name="email" />
									<br />
									<br />
									<label>验证码：</label>
									<input class="itxt" type="text" style="width: 150px;" name="code" value="${param.code }"/>
									<img alt="" src="code.jpg" style="float: right; margin-right: 40px;width:90px;height:45px" id="code_img">									
									<br />
									<br />
